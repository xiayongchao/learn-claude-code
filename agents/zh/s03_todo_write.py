...
SYSTEM = f"""你是工作目录 {WORKDIR} 下的编程智能体。
使用待办工具规划多步骤任务。开始前标记为进行中，完成后标记为已完成。
优先使用工具操作，而非文字说明。"""
...
class TodoManager:
    ...
    def update(self, items: list) -> str:
        ...
        for i, item in enumerate(items):
            ...
            if not text:
                raise ValueError(f"待办事项 {item_id}: 文本不能为空")
            if status not in ("pending", "in_progress", "completed"):
                raise ValueError(f"待办事项 {item_id}: 状态无效 '{status}'")
            if status == "in_progress":
                in_progress_count += 1
            validated.append({"id": item_id, "text": text, "status": status})
        if in_progress_count > 1:
            raise ValueError("同一时间仅允许一个任务处于进行中状态")
    ...
    def render(self) -> str:
        ...
        for item in self.items:
            marker = {"待处理": "[ ]", "处理中": "[>]", "已完成": "[x]"}[item["status"]]
...
TOOLS = [
    ...
    {"type": "function", "function": {"name": "todo", "description": "更新任务列表，追踪多步骤任务的执行进度。",
